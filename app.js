      ...
      Event.find({
        ...
        if (err) {
          ...
          if (eventReturn.slots == "Yes") {
            ...
            Event.collection.insert(shiftArrays, {
              ordered: false
            }, function (err, shifts) {
              if (err) console.log(err);
              else {
...
app.get("/my-events", isMember, function (req, res) {
  ...
  User.findById(req.user._id).populate("events").exec(function (err, events) {
    if (err) console.log(err);
      ...
      User.findById(userID).populate("events").populate("wentEvents").exec(function (err, user) {
        if (err) console.log(err);
        else res.render("member/myEvents", {
          events: user.events,
          wentEvents: user.wentEvents,
          EEvents: EEvents
        });
...
app.get('/appeals', function (req, res) {
  ...
  User.findById(req.user._id).populate("events").sort({
    submissionDate: 'asc'
  }).populate("appeals").exec(function (err, foundUser) {
    if (err) console.log(err);
    else {
      EEvents = foundUser;
...
app.get("/lunch-buddy", function (req, res) {
  ...
  User.findById(req.user._id).populate("events").exec(function (err, events) {
    if (err) console.log(err);
      ...
      Event.find({
        lunchBuddy: true
      }).populate('lunchBuddyIds').exec(function (err, lunchBuddyEvents) {
        ...
        if (err) console.log(err);
          ...
          Appeal.find({}).exec(function (err, AllAppeals) {
            ...
            Event.find({
              ...
              Event.find({
                eventName: /True Life/
              }, '_id', function (err, trueLifeEvents) {
                User.find({
                  role: "member"
                }, function (err, members) {
                  var condition = false;
...
app.post("/appeals/:appealID/accepted", function (req, res) {
  var appealID = req.params.appealID;
  Appeal.findByIdAndUpdate(appealID, {
...
app.get("/staff/lunch-buddy/:id/pair", function (req, res) {
  ...
  User.find({}, function (err, users) {
    ...
    LunchBuddy.findById(req.params.id).exec(function (err, foundLunchBuddy) {
      ...
      Event.collection.insert(meetingArrays, {
        ordered: false
