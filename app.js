...
app.use(bodyParser.urlencoded({extended:true})); // Getting inputs from form and posting data
...
// Event.deleteMany({lunchBuddy:true},function(err){
//   if(err) console.log(err);
//   else{
//     console.log("worked");
//   }
...
//                                 GENERAL ROUTES
//==============================================================================
app.get('/.well-known/acme-challenge/AkplRW-XS0PXqwWLaVFs4V3FaykUC_ztXvlr6f9ZPno',function(req, res) {
    res.send("AkplRW-XS0PXqwWLaVFs4V3FaykUC_ztXvlr6f9ZPno.JdgXhI9HBLARao1b6jB1UpiS9rDT1hjFqtqd7UzZF5U");
})
...
app.get("/home", function(req, res){
  User.findOne({position:"President"},function(err,president){
    if(err) console.log(err);
      ...
      User.findOne({position:"Vice President"},function(err,vicePresident){
        if(err) console.log(err);
        else{
            
           res.render("landing/home",{president:president,vicePresident:vicePresident});
          }
      });
    }
...
app.get("/staff", function(req, res){
  ...
  User.find({role:"staff"},function(err,staff){
    console.log(staff);
    res.render("landing/staff",{staff:staff,pictures:pictures});
  })
}); 
...
app.get("/committees/SAAM",function(req, res) {
  User.findOne({position:"SAAM Executive"},function(err,executive){
        if(err) console.log(err);
        else{     
            res.render("landing/SAAM",{executive:executive});
  }})
});

app.get("/committees/DEVO",function(req, res) {
  User.findOne({position:"DEVO Executive"},function(err,executive){
        if(err) console.log(err);
        else{     
            res.render("landing/DEVO",{executive:executive});
  }})
});

app.get("/committees/RAMH",function(req, res) {
  User.findOne({position:"RAMH Executive"},function(err,executive){
        if(err) console.log(err);
        else{     
            res.render("landing/RAMH",{executive:executive});
  }})
});

app.get("/committees/SHIP",function(req, res) {
  User.findOne({position:"SHIP Executive"},function(err,executive){
        if(err) console.log(err);
        else{     
            res.render("landing/SHIP",{executive:executive});
  }})
});
...
// -------------------------------- APPLY-----------------------------------
 ...
 app.get("/admissions", function(req, res){
    ...
    Application.findOne().sort({created_at: -1}).exec(function(err, foundApp) {
      if(err) console.log(err);
      else{
          res.render("landing/apply",{applications:foundApp}); 
        };
    });
});
...
app.post('/contact', function (req, res) {
      ...
      var text = senderName + senderEmail+ senderSubject + '<p>Body:&nbsp; '+req.body.text+'<p>';
      ...
      transporter.sendMail(mailOptions, (error, info) => {
          ...
          req.flash("check","Our staff will get back to you as soon as possible. Thank fou for contacting Us!");
...
app.post("/member-sign-up",isMemberGoodCode, function(req, res){
...
function isStaffGoodCode(req, res, next){
   if((req.body.accessCode == "848619" && req.body.position.includes("Director")) || (req.body.accessCode == "489638" && (req.body.position.includes("President") || req.body.position.includes("Executive"))) ){
    return next();
   } else{
   res.redirect("/error");}
}
...
app.post("/staff-sign-up",isStaffGoodCode, function(req, res){
    User.register(new User({username: req.body.username, 
                            phoneNumber: req.body.phoneNumber,
                            firstName:req.body.firstName,
                            lastName:req.body.lastName,
...
app.get("/logging-in", function(req, res){
    ...
    res.render("landing/loader",{role:role});
}); 
...
//------------------------------------------------------------------------------
 ...
 console.log(req.user.firstName + " " + req.user.lastName + " just logged in");
    ...
    Event.find({wasSubmitted:false}, function(err, events){
        if(err){
             console.log(err);
               } else {
                  events.forEach(function(event){
                    ...
                    justEvents.sort(function(a, b){return a.eventDate - b.eventDate}); 
                        ...
                        Event.find({wasSubmitted:false}, function(err, events){
                           if(err){
                               ...
                               events.forEach(function(event) {
                                    ...
                                    d = d.getTime(); 
                                    m = m.getTime(); 
                                    if( d < (m+86500000)  && (event.numberSpots > 0 || event.numberSpots == null )) counter += 1;
                               });
                               eventsLength = counter;
                                ...
                                Announcement.find({},function(err, announcements) {
                                    if(err) console.log(err);
                                        ...
                                        EEvents:EEvents,currentEvent:currentEvent,eventsLength:eventsLength,announcements:announcements,goals:goals, 
...
app.get("/points/new",isMember,function(req, res) {
   ...
   User.findById(req.user._id).populate("events").exec(function(err,events){
        if (err) console.log(err);
            ...
            EEvents = events;var justEvents = events.events;justEvents.sort(function(a, b){return a.eventDate - b.eventDate}); 
            res.render("member/submitPoint",{user:req.user,EEvents:EEvents}); 
...
app.post("/points",isMember,function(req, res) {
    ...
    Event.create(event,function(err, submittedEvent) {
        if(err) console.log(err);
            ...
            req.flash("success"," Your point request has successfully been submitted.");
            req.flash("check","Check this page later for a decision from the Membership Executives.");
...
app.get("/points/view",isMember,function(req, res) {
    ...
    User.findById(req.user._id).populate("events").exec(function(err,events){
            if (err) console.log(err);
                ...
                EEvents = events;var justEvents = events.events;justEvents.sort(function(a, b){return a.eventDate - b.eventDate});
                 User.findById(req.user._id).populate("serviceEvents").populate("socialEvents").populate("programmingEvents").exec(function(err,user){
                if (err) console.log(err);
                else{   
                        var serviceEvents = user.serviceEvents;
                        var socialEvents = user.socialEvents;
                        var programmingEvents = user.programmingEvents;
                        res.render("member/myPoints",{goals:goals,EEvents:EEvents,user:user}); }
            });
            }
        })
...
app.get("/event",isLoggedIn,function(req, res) {
    ...
    User.findById(req.user._id).populate("events").exec(function(err,events){
        if (err) console.log(err);
            ...
            EEvents = events;
            var justEvents = events.events;justEvents.sort(function(a, b){return a.eventDate - b.eventDate});
            
               
          Event.find({wasSubmitted:false,lunchBuddy:false}).sort({eventDate: 'asc'}).exec(function(err, events) {
              if(err) console.log(err);
              else{ 
               ...
               if(err){
                   console.log(err);
                          ...
                          eventsLength = counter;
                          events.sort(function(a, b){return a.eventDate - b.eventDate}); 
                           
                          res.render("member/viewEvents",{userEvents:userEvents,
                          events:events,EEvents:EEvents});
                       }
                  }
            });
        }
...
app.get("/event/create",isLoggedIn,function(req, res) {
   ...
   User.findById(req.user._id).populate("events").exec(function(err,events){
        if (err) console.log(err);
            ...
            EEvents = events;
            var justEvents = events.events;justEvents.sort(function(a, b){return a.eventDate - b.eventDate});
            res.render("member/createEvent",{EEvents:EEvents}); 
        }
    });
});
//==============================================================================
       ...
       if(err){
           ...
           events.forEach(function(event) {
            ...
            if(eventReturn.slots == "Yes"){
              ...
              Event.collection.insert(shiftArrays, { ordered: false },function(err,shifts){
                if(err) console.log(err);
                else{
 ...
 var userID = req.user._id;
   ...
   User.findById(req.user._id).populate("events").exec(function(err,events){
        if (err) console.log(err);
            ...
            EEvents = events;
            var justEvents = events.events;justEvents.sort(function(a, b){return a.eventDate - b.eventDate});
            User.findById(userID).populate("events").populate("wentEvents").exec(function(err,user){
                if (err) console.log(err);
                  else{ 
                      var eventID = req.params.id;
                      Event.findById(eventID).populate("slotsIDs").exec(function(err,event){
                        var shifts = event.slotsIDs;
                        res.render("staff/shifts",{event:event,shifts:shifts,events:user.events,wentEvents:user.wentEvents,EEvents:EEvents});
                      });
                    }
                });
            }
    ...
    if(err) console.log(err);
       ...
       User.findById(req.user._id).populate("events").exec(function(err,events){
        if (err) console.log(err);
          ...
          var EEvents = events;
          //console.log(EEvents);
          res.render("staff/shiftTable",{shifts:event.slotsIDs,EEvents:EEvents});
        };
      });
          ...
          console.log("problem 1");
            User.findById(userID,function(err, foundUser){
               if(err) console.log(err); 
                   ...
                   var x;
                   if(foundEvent.numberSpots != null) x = foundEvent.numberSpots-1;
                   else x = null;
                   
                   //Updating number of spots
                   Event.findByIdAndUpdate(eventID,{numberSpots:x },function(err,data){
                       if(err) console.log(err);
                       else {
            ...
            //Push Event ID inside USER's wentEvent array and specific event array
                ...
                Event.findById(eventID,'pointType',function(err,event){
                    if(err) console.log(err);
                        ...
                        if(event.pointType == "Service"){ foundUser.serviceEvents.push(eventID); }
                        if(event.pointType == "Social"){ foundUser.socialEvents.push(eventID); }
                        if (event.pointType == "Programming"){foundUser.programmingEvents.push(eventID); }
                    ...
                    // foundUser.wentEvents.sort({eventDate:'asc'});
                     //Delete Event ID from current event array
                        ...
                        if (index > -1) { foundUser.events.splice(index, 1); }
                    ...
                    foundUser.save(function(err,data){
                        if(err) console.log(err);
                                ...
                                req.flash("check","Remember, the more events you go to the higher your chances of winning the prize.");
        ...
        if(err) console.log(err);
            ...
            Event.findById(eventID,function(err, foundEvent) {
                ...
                if(foundEvent.numberSpots != null){
                    Event.findByIdAndUpdate(eventID,{numberSpots:(foundEvent.numberSpots + 1)},function(err,foundEvent){
                        if(err) console.log(err);
                        else{
                            foundEvent.save();
                        }
                ...
                if (index1 > -1) { foundEvent.userIDs.splice(index1, 1); } 
                foundEvent.save();
...
app.get("/my-events",isMember,function(req, res) {
   ...
   User.findById(req.user._id).populate("events").exec(function(err,events){
        if (err) console.log(err);
            ...
            User.findById(userID).populate("events").populate("wentEvents").exec(function(err,user){
                               if (err) console.log(err);
                               else res.render("member/myEvents",{events:user.events,wentEvents:user.wentEvents,EEvents:EEvents});
            });
        }
    });
...
app.get("/appeals/new",function(req, res) {
   ...
   User.findById(req.user._id).populate("events").exec(function(err,events){
        if (err) console.log(err);
            ...
            EEvents = events;var justEvents = events.events;justEvents.sort(function(a, b){return a.eventDate - b.eventDate});
            res.render("member/submitAppeal",{user:req.user,EEvents:EEvents}); 
        ...
        User.findById(requesterID,function(err, foundUser) {
           if(err) console.log(err);
               ...
               // Associating appeal to requester
                ...
                Appeal.find({staffDecision:null},function(err, appeals) {
                    if(err) console.log(err);
                    else{
                        currentAppeals = appeals.length;
                                      ...
                                      User.find({position:"Membership Executive"},function(err,membershipExecs){
                                        ...
                                        transporter.sendMail(mailOptions);
                                          ...
                                          req.flash("success"," Your appeal has successfully been submitted.");
                                          req.flash("check","Check this page later for a decision from the Membership Executives.");
...
app.get('/appeals',function(req, res) {
    ...
    User.findById(req.user._id).populate("events").sort({submissionDate:'asc'}).populate("appeals").exec(function(err,foundUser){
        if (err) console.log(err);
        else{
            EEvents = foundUser;var justEvents = foundUser.events;justEvents.sort(function(a, b){return a.eventDate - b.eventDate});
            
               
          Event.find({wasSubmitted:false}).sort({eventDate: 'asc'}).exec(function(err, events) {
              if(err) console.log(err);
              else{ 
               ...
               if(err){
                   console.log(err);
                           ...
                           events.forEach(function(event) {
                                ...
                                m = event.eventDate ;
                                d = d.getTime(); m = m.getTime(); 
                                if( (d-43200000)  < m && (event.numberSpots > 0 || event.numberSpots == null )) counter += 1;
                           });
                           eventsLength = counter;
                           
                          res.render("member/viewAppeals",{userEvents:userEvents,
                          events:events,EEvents:EEvents,appeals:foundUser.appeals});
                       }
                    }
            });
        }
...
app.get("/guidelines",isLoggedIn,function(req, res) {
   ...
   User.findById(req.user._id).populate("events").exec(function(err,events){
        if (err) console.log(err);
          ...
          Guideline.find({category:'Points'},function(err,pointsGuidelines){
            if(err) console.log(err);
            else{
              Guideline.find({category:'Appeals'},function(err,appealsGuidelines){
                if(err) console.log(err);
                else{
                  Guideline.find({category:'Drinking'},function(err,drinkingGuidelines){
                    if(err) console.log(err);
                    else{
                      EEvents = events;var justEvents = events.events;justEvents.sort(function(a, b){return a.eventDate - b.eventDate});
                      res.render("member/guidelines",{pointsGuidelines:pointsGuidelines,appealsGuidelines:appealsGuidelines,
...
app.get("/faqs",isLoggedIn,function(req, res) {
   ...
   User.findById(req.user._id).populate("events").exec(function(err,events){
        if (err) console.log(err);
          ...
          Guideline.find({category:'FAQs'},function(err,faqs){

            EEvents = events;var justEvents = events.events;
            justEvents.sort(function(a, b){return a.eventDate - b.eventDate});
            res.render("member/questions",{faqs:faqs,user:req.user,EEvents:EEvents}); 
        ...
        if (err) console.log(err);
            ...
            Form.find({},function(err, allForms) {
                if(err) console.log(err);
                   ...
                   res.render("member/forms",{user:req.user,EEvents:EEvents,
                               forms:allForms});  
                }
            });
        }
...
app.get("/contact-sheet",isLoggedIn,function(req, res) {
   ...
   User.findById(req.user._id).populate("events").exec(function(err,events){
        if (err) console.log(err);
            ...
            EEvents = events;var justEvents = events.events;justEvents.sort(function(a, b){return a.eventDate - b.eventDate});
                res.render("member/contactSheet",{ EEvents:EEvents,
                }); 
            ...
            Event.find({wasSubmitted:false}, function(err, events){
               if(err){
                   console.log(err);
                    ...
                    res.render("member/calendar",{EEvents:EEvents}); 
               }
            });
        }});
});
   ...
   var EEvents; 
    User.findById(req.user._id).populate("events").exec(function(err,events){
        if (err) console.log(err);
            ...
            EEvents = events;var justEvents = events.events;justEvents.sort(function(a, b){return a.eventDate - b.eventDate});
                res.render("member/googleDrive",{EEvents:EEvents,currentAppeals:currentAppeals,submittedPointsLength:submittedPointsLength,
                  }); 
   ...
   var EEvents; 
    User.findById(req.user._id).populate("events").exec(function(err,events){
        if (err) console.log(err);
                ...
                Event.find({lunchBuddy:true}).populate('lunchBuddyIds').exec(function(err,lunchBuddyEvents){

                  if(err) console.log(err);
                  else{
                    res.render("member/lunchBuddy",{EEvents:EEvents,lunchBuddyEvents:lunchBuddyEvents});
                  }
            });
         }
      }); 
    ...
    User.findById(req.user._id).populate("events").exec(function(err,events){
        if (err) console.log(err);
          ...
          Course.find({}).exec(function(err,allCourses){
            EEvents = events;var justEvents = events.events;justEvents.sort(function(a, b){return a.eventDate - b.eventDate});
                res.render("member/studyGroups",{allCourses:allCourses,user:req.user,EEvents:EEvents
                }); 
...
app.post("/study-groups",function(req, res) {
  ...
  User.findById(req.user._id,function(err,user){
    if(err) console.log(err);
       ...
       var course = req.body.courseCode.toUpperCase() + "-" + req.body.courseNumber;
        ...
        Course.find({},function(err,courses){
          if(err) console.log(err);
            ...
            courses.forEach(function(existingCourse){
              ...
              if(course == existingCourse.courseID){ 
                ...
                existingCourse.users.push({name:req.user.firstName+" "+req.user.lastName,profilePicture:req.user.profilePicture}); 
                existingCourse.save();
                condition = true;}
              });
                //If course doesn't exist then create it and add user.
                if(condition == false){
                  var newCourse = {courseID : course};
                  Course.create(newCourse,function(err, newlyCreatedCourse){
                    if(err) console.log(err);
                    else{
                      newlyCreatedCourse.users.push({name:req.user.firstName+" "+req.user.lastName,profilePicture:req.user.profilePicture});
          ...
          } else {req.flash("success",course + " has successfully been added.");res.redirect('/study-groups');};
        };
      }); 
    };
  })
  ...
  User.findById(req.user._id,function(err,user){
    if(err) console.log(err);
      ...
      for(var i = 0; i < user.courses.length; ++i){
        if(user.courses[i] == course){user.courses.splice(i,1); user.save();}}
        // Removing Name of user from correspondig courses Array
        Course.findOne({courseID:course},function(err,foundCourse){
          for(var i = 0; i < foundCourse.users.length; ++i){
            var username = req.user.firstName + " " + req.user.lastName;
            if(foundCourse.users[i].name == username){foundCourse.users.splice(i,1); foundCourse.save();
            }
          }     
        });
...
app.get('/have-read-constitution',isMember,function(req,res){
  User.findByIdAndUpdate(req.user._id,{haveReadConstitution:true},function(err,user){
    if(err) console.log(err);
    else{
      req.flash("success","Thank you for taking the time to read the constitution!");
      res.redirect('back');
    }
  })
});
...
app.get('/home-page',function(req, res) {
  console.log(req.user.firstName + " " + req.user.lastName + " just logged in");
    User.find({role:"member"},function(err, members) {
        if(err) console.log(err);
        else{   
                Event.find({wasSubmitted:false},function(err, events) {
                    if(err) console.log(err);
                    ...
                    Appeal.find({}).exec(function(err,AllAppeals){
                      ...
                      AllAppeals.forEach(function(appeal){
                        ...
                        if(appeal.eventDate != null){
                          if(appeal.eventDate.getTime() >= today) neededAppeals.push(appeal);
                        }
                        var today = new Date();
                      });
                      Event.find({lunchBuddy:false,pointType:{$ne:"Programming"},eventDate:{$gt:today}},function(err,currentEvents){
                         Event.find({eventName:/True Life/},'_id',function(err,trueLifeEvents){
                          User.find({role:"member"},function(err,members){
                          var condition = false;
                          var haveNotSignedUp = [];
                          for(var z = 0; z < members.length; ++z){
                              for(var i = 0; i < members[z].events.length; ++i){
                                for(var j = 0; j < trueLifeEvents.length; ++j){
                                  if(members[z].events[i].toString() == trueLifeEvents[j]._id.toString()){
                                    condition = true;
                                    break;
                                  }
                                }
                              }
                             if(condition = false) haveNotSignedUp.push(members[z]);
                          }
                         
                
                            res.render('staff/homepage',{appeals:neededAppeals,currentEvents:currentEvents});
                          });
...
app.get("/event/view/:id",function(req, res) {
    ...
    var wentUsers;
    var missedUsers;
    Event.findById(eventID).populate("wentUsers").populate("missedUsers").populate("userIDs").exec(function(err,foundEvent){
        if(err) console.log(err);
        else{
            signedUpUsers = foundEvent.userIDs;
            res.render("staff/specificEvent",{signedUpUsers:signedUpUsers,event:foundEvent,eventID:eventID});
...
app.get("/announcements",function(req, res) {
    ...
    Announcement.find({},function(err, announcements) {
        if (err) console.log(err);
        res.render("staff/announcement",{announcements:announcements});
    });
});
//==================================================================
...
User.find({role:"member"}).sort({lastName: 'asc'}).exec(function(err,members){
    if(err) console.log(err);
        Meeting.find({}).exec(function(err,meetings){
            if (err) console.log(err);
            else{
               res.render("staff/attendance",{members:members,meetings:meetings,requestee:req.user}); 
            }
        });
       ...
       if(err) console.log(err);
         ...
         User.find({role:"member"}).sort({lastName: 'asc'}).exec(function(err,members){
            if(err) console.log(err);
                    ...
                    Meeting.findById(meetingID).populate("presentIDs").populate("absentIDs").exec(function(err,newfoundMeeting){
                        if(err) console.log(err);
                        foundMeeting = newfoundMeeting;
                        res.render("staff/specificMeeting",{members:members,foundMeeting:foundMeeting});
                    });
        ...
        if(err) console.log(err);
            ...
            User.findById(userID,function(err, foundUser) {
                if(err) console.log(err);
                    ...
                    User.findByIdAndUpdate(userID,{meetingPoints:meetingPoints},function(err, user) {
                        if(err) console.log(err);
                        else{
...
app.get("/members",function(req, res) {
    ...
    User.find({role:"member"}).sort({firstName:"asc"}).exec(function(err,foundUsers){
        if(err) console.log(err);
        else{
            res.render("staff/members",{members:foundUsers});
        }
    });
});
//==================================================================
app.get("/members/:id",function(req, res) {
    ...
    User.findById(memberID).populate("events").populate("wentEvents").populate("programmingEvents").
        populate("socialEvents").populate("serviceEvents").populate("appeals").exec(function(err,foundUser){
        if(err) console.log(err);
        else{
            res.render("staff/specificMember",{member:foundUser});
        }
    });
});
// --------------------------------------APPEALS-----------------------------
...
app.get("/staff/appeals",function(req, res) {
    Appeal.find({}).sort({submissionDate:'desc'}).exec(function(err, appeals) {
        if(err) console.log(err);
        else{
          appeals.sort(function(a, b){return b.submissionDate - a.submissionDate});
            res.render("staff/appeals",{appeals:appeals});
        }
    });
});
//=====================================================================
app.get("/staff/appeal/:appealID",function(req,res){
  ...
  Appeal.findById(appealID,function(err,appeal){
    res.render("staff/specificAppeal",{appeal:appeal});
  })
});
...
app.post("/appeals/:appealID/accepted",function(req, res) {
    var appealID = req.params.appealID;
    Appeal.findByIdAndUpdate(appealID,{comment:req.body.comment},function(){
          Appeal.findByIdAndUpdate(appealID,{staffDecision:"Accepted"},function(err, appeal) {
        if(err) console.log(err);
        else{
            // Sending out email to let them about decision
         
       
            var mytext = "<h1>EVENT APPEAL DECISION<h1> <p>This is to let you know that your appeal has been ACCEPTED for " +
                      "the following event:&nbsp;</p>" + 
                       "<p> Event Name:&nbsp;" + appeal.eventName + "</p>" +
                       "<p> Event Date:&nbsp; " + appeal.eventDate.toDateString() + "</p>" +
                       "<p> Event Type:&nbsp; " + appeal.eventType + "</p>" +
                       "<p> Comment:&nbsp;" + appeal.comment + "</p>"  + 
            ...
            transporter.sendMail(mailOptions);
                ...
                req.flash("check",appeal.requesterName +" has been notified of decision via email and via his portal.");
...
app.post("/appeals/:appealID/denied",function(req, res) {
  ...
  Appeal.findByIdAndUpdate(appealID,{comment:req.body.comment},function(){
          Appeal.findByIdAndUpdate(appealID,{staffDecision:"Denied"},function(err, appeal) {
        if(err) console.log(err);
        else{


             var mytext = "<h1>EVENT APPEAL DECISION<h1> <p>This is to let you know that your appeal has been DENIED for " +
                      "the following event:&nbsp;</p>" + 
                       "<p> Event Name:&nbsp;" + appeal.eventName + "</p>" +
                       "<p> Event Date:&nbsp; " + appeal.eventDate.toDateString() + "</p>" +
                       "<p> Event Type:&nbsp; " + appeal.eventType + "</p>" + 
...
app.get("/staff/points/view",function(req, res) {
    ...
    User.find({role:"member"}).sort({lastName:"asc"}).exec(function(err,foundUsers){
        if(err) console.log(err);
        else{
            Goal.findOne().sort({created_at: -1}).exec(function(err, foundGoal) {
                if(err) console.log(err);
                else{
                    goals  = [foundGoal.socialGoal,foundGoal.serviceGoal,foundGoal.programmingGoal,foundGoal.meetingGoal];
                    res.render("staff/viewPoints",{staff:req.user,members:foundUsers,goals:goals});
                }
            });
        }
    });
...
app.get("/staff/submitted-points",function(req, res) {
    ...
    Event.find({wasSubmitted:true}).sort({eventName:"asc"}).exec(function(err, submittedEvents) {
        if(err) console.log(err);
        else{
            res.render("staff/submittedPoints",{submittedEvents: submittedEvents,staff:req.user});
        }
    })
});
...
app.get("/staff/submitted-points/:eventID",function(req, res) {
    ...
    Event.findById(eventID,function(err, event) {
        if(err) console.log(err);
            ...
            res.render("staff/specificPoint",{event: event,staff:req.user});
        }
    })
});
//==================================================================
app.post("/point/:userID/:eventID/:eventType/accept",function(req, res) {
    ...
    User.findById(req.params.userID,function(err, foundUser) {
        if(err) console.log(err);
            ...
            if(req.params.eventType == "Social") foundUser.socialEvents.push(req.params.eventID);  
            if(req.params.eventType == "Service") foundUser.serviceEvents.push(req.params.eventID); 
            if(req.params.eventType == "Programming") foundUser.programmingEvents.push(req.params.eventID); 
            foundUser.save();
            
            Event.findByIdAndUpdate(req.params.eventID,{comment:req.body.comment,},function(err, foundEvent) {
                if(err) console.log(err);
                else{
                    

                var mytext = "<h1>POINT SUBMISSION DECISION<h1> <p>This is to let you know that your point submission has been ACCEPTED for " +
                  "the following event:&nbsp;</p>" + 
                  "<p > Event Name:&nbsp;" + foundEvent.eventName + "</p>" +
                  "<p> Event Date:&nbsp; " + foundEvent.eventDate.toDateString() + "</p>" +
                  "<p> Event Location:&nbsp; " + foundEvent.Location + "</p>" + 
                  "<p> Event Type:&nbsp; " + foundEvent.pointType + "</p>" + 
                  "<p> Comment:&nbsp;" + foundEvent.comment+ "</p>" +
                ...
                html: mytext // html body
                     ...
                     transporter.sendMail(mailOptions);
                      foundEvent.status = "Approved";
                       ...
                       req.flash("check",foundEvent.creatorName +" has been notified of your decision via email and via his portal.");
    ...
    User.findById(req.params.userID,function(err, foundUser) {
        if(err) console.log(err);
          ...
          Event.findByIdAndUpdate(req.params.eventID,{comment:req.body.comment},function(err, foundEvent) {
              ...
              var mytext = "<h1>POINT SUBMISSION DECISION<h1> <p>This is to let you know that your point submission has been DENIED for " +
                  "the following event:&nbsp;</p>" + 
                  "<p> Event Name:&nbsp;" + foundEvent.eventName + "</p>" +
                  "<p> Event Date:&nbsp; " + foundEvent.eventDat.toDateString() + "</p>" +
                  "<p> Event Location:&nbsp; " + foundEvent.Location + "</p>" + 
                  "<p> Event Type:&nbsp; " + foundEvent.pointType + "</p>" + 
                  "<p> Comment:&nbsp;" + foundEvent.comment+ "</p>"  ;
         ...
         Goal.findOne().sort({created_at: -1}).exec(function(err, foundGoal) {
             if(err) console.log(err);
                 ...
                 Goal.findByIdAndUpdate(goalID,{  
                    ...
                    meetingGoal:req.body.meetingGoal},function(err,changedGoals){
                     if(err) console.log(err);
                     else{ 
                          goals  = [changedGoals.socialGoal,changedGoals.serviceGoal,changedGoals.programmingGoal,changedGoals.meetingGoal];
...
app.get("/staff/forms",function(req, res) {
    ...
    Form.find({},function(err, allForms) {
        if(err) console.log(err);
            ...
            res.render("staff/forms",{currentAppeals:currentAppeals,
                forms: allForms
            });
        }
    });
          ...
          if(list1[index].role != "staff" && list2[index].role != "staff"){
           ...
           index -= 1;
            ...
            while((list1[index].role == "staff" && list2[index].role == "staff")){
              list1.sort(function() { return 0.5 - Math.random() });
              list2.sort(function() { return 0.5 - Math.random() });
            }
     ...
     // Removing old Lunchbuddies and Adding IDs of events in lunch Buddy Array for each Member
      ...
      // Array used to store meetings created for each pair
        var meetingArrays = [];
         ...
         for(var i = 0; i < pairs.length; ++i){
           // Creating events for each pair
            ...
            var description = "Howdy " + pairs[i][0].firstName + "! Howdy " + pairs[i][1].firstName + "! You guys are Lunch Buddies for " + foundLunchBuddy.title +"! So get excited! Whooooop! Make sure to contact each other in order to set up a time and place so that you guys can meet. We have provided the contact informations. We want really want sophomores to get closer this year. So enjoy your meal!"
            var date = new Date();
            ...
            var event = {
              eventName : "Lunch Buddy " + foundLunchBuddy.title + ": " +pairs[i][0].firstName + " & " +pairs[i][1].firstName ,
              lunchBuddy: true,
              eventDate : date,
        ...
        // insert functions allows to create many collections at once
             Event.collection.insert(meetingArrays, { ordered: false },function(err,meetings){
              req.flash("success","Members have successfully been paired for " + foundLunchBuddy.title);
              req.flash("check"," They will have seven days to meet then they will be able to earn points.")
...
app.get('/staff/lunch-buddy/:id',function(req,res){
  LunchBuddy.findById(req.params.id).populate('groupA').populate('groupB').exec(function(err,lunchBuddy){
    if(err) console.log(err);
    else{
      ...
      for(var i = 0; i < lunchBuddy.groupA.length; ++i){
          formattedLunchBuddy.push([lunchBuddy.groupA[i],lunchBuddy.groupB[i]]);
        }
     res.render('staff/specificLunchBuddy',{lunchBuddy:lunchBuddy,formattedLunchBuddy:formattedLunchBuddy});
    }
  })
});
...
app.get('/staff/lunch-buddy',function(req,res){
  LunchBuddy.find({},function(err,lunchBuddies){
    if(err) console.log(err); 
      ...
      res.render("staff/lunchBuddy",{lunchBuddies:lunchBuddies});
    };
  });
});
//============================================================================
app.post('/staff/lunch-buddy',function(req,res){
  ...
  LunchBuddy.create(lunchBuddy,function(err,createdLunchBuddy){
    if(err) console.log(err);
      ...
      req.flash("success", createdLunchBuddy.title + " has been successfully created.")
  ...
  User.find({},function(err,users){
    ...
    Application.findOne().sort({created_at: -1}).exec(function(err, foundApp) {
      if(err) console.log(err);
            ...
            res.render("staff/settings",{applications:foundApp,users:users})
          }
      });
    }
  });
        ...
        if(err) console.log(err);
          ...
          allCourses.forEach(function(course){
            for(var i = 0; i < course.users.length; ++i){
              if(course.users[i] == user.firstName + " " + user.lastName) course.users.splice(i,1) ;
            }
          });
          req.flash("success","Account has successfully been deleted!");
          req.flash("check", user.firstName + " won't be able to access his or her account anymore.")
...
app.post("/staff/settings/biography",function(req,res){
  ...
  User.findByIdAndUpdate(userID,{biography:biography},function(err,user){
    if(err) console.log(err);
    else{
...
app.post("/staff/settings/president-message",function(req,res){
  ...
  User.findByIdAndUpdate(userID,{presidentMessage:presidentMessage},function(err,user){
    if(err) console.log(err);
    else{
      req.flash("success","Your President Message has successfully been updated!");
...
app.post("/staff/settings/vp-message",function(req,res){
  ...
  User.findByIdAndUpdate(userID,{vpMessage:vpMessage},function(err,user){
    if(err) console.log(err);
    else{
...
app.post("/staff/settings/executive-message",function(req,res){
  ...
  User.findByIdAndUpdate(userID,{executiveMessage:executiveMessage},function(err,user){
    if(err) console.log(err);
    else{
      req.flash("success","Your Executive Message has successfully been updated!");
...
app.post("/staff/settings/release-app",function(req,res){
  ...
  Application.findOne().sort({created_at: -1}).exec(function(err, foundApp) {
      if(err) console.log(err);
        ...
        req.flash("success", appType + " application has successfully been released.");
        req.flash("check","Check admissions page to make sure it has been updated accordingly.");
        res.redirect('/staff/settings');   
    };
  });
});
  ...
  Application.findOne().sort({created_at: -1}).exec(function(err, foundApp) {
      if(err) console.log(err);
        ...
        if(appType == "Director"){
          ...
          req.flash("success", appType + " application has successfully been removed.");
          req.flash("check","Check admissions page to make sure it has been updated accordingly.");
          res.redirect('/staff/settings');   
    };
  });
  ...
  Application.findOne().sort({created_at: -1}).exec(function(err, foundApp) {
      if(err) console.log(err);
        ...
        if(appType == "Director"){
          ...
          req.flash("success", appType + " application has successfully been changed.");
          req.flash("check","Check admissions page to make sure it has been updated accordingly.");
          res.redirect('/staff/settings');   
    });
});
...
app.get("/staff/guidelines",function(req, res) {
    ...
    Guideline.find({},function(err,guidelines){
          res.render("staff/guidelines",{guidelines:guidelines});
    })
});
//========================================================s
app.post("/staff/guidelines",function(req, res) {
  ...
  Guideline.create(guideline,function(err,newGuideline){
    if(err) console.log(err);
      ...
      req.flash("success", category + " Guideline has successfully been created.");
      req.flash("check","Sophomores will be able to view it on their portals.");
      ...
      if(err);
        ...
        users.forEach(function(user){
          formattedLinks.forEach(function(link){
            if(link.toUpperCase().includes(user.firstName.replace(/ /g,'').toUpperCase()) && link.toUpperCase().includes(user.lastName.replace(/ /g,'').toUpperCase())){
...
console.log(link[0]);
    ...
    User.findById(req.user._id,function(err,user){
      if(err);
            ...
            if(link[0].toUpperCase().includes(user.firstName.replace(/ /g,'').toUpperCase()) && link[0].toUpperCase().includes(user.lastName.replace(/ /g,'').toUpperCase())){
